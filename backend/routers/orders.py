...
Frontend uses a flat single-product order format:
- POST /orders body:
  {
    customer_name,
    customer_email,
...
Database still uses Order + OrderDetail structure.
This router converts DB data into the flat format expected by the frontend.
"""
...
def _serialize_order(db: Session, order: Order) -> dict:
    """
    Converts Order + first OrderDetail into the flat frontend format.
    """
...
def create_order(
    ...
    db: db_dependency,
    user: dict = Depends(
        require_roles(ROLE_ADMIN, ROLE_BUSINESS_OWNER, ROLE_SALES_MANAGER)
    ),
):
    """
    Creates a new order and decreases product stock if the order is not cancelled.
    """
...
def update_order_status(
    ...
    """
    Updates order shipping/status information.
    Frontend Update button uses this endpoint.
    """
...
def delete_order(
    ...
    """
    Deletes an order.
    If the order was not cancelled, product stock is restored before deletion.
